...
import jpg.k.simplyimprovedterrain.util.SplerpBase;
import jpg.k.simplyimprovedterrain.world.deposit.DisklessDepositor;
...
public abstract class NoiseChunkGeneratorImproved<T extends GenerationSettings> extends ChunkGenerator<T> {
    ...
    private FastSimplexStyleNoise[] fssnOctaves1;
    private FastSimplexStyleNoise[] fssnOctaves2;
    private FastSimplexStyleNoise[] fssnOctavesBlend;
    ...
    private SplerpBase biomeSplerpBase;
    ...
    public NoiseChunkGeneratorImproved(IWorld worldIn, BiomeProvider biomeProvider, int horizontalBiomeGranularityPowerOfTwo, int worldHeight, T settings, int nOctavesOutput, int nOctavesBlend, double hScale, double vScale, double hScaleBlend, double vScaleBlend) {
        ...
        fssnOctaves1 = new FastSimplexStyleNoise[nOctavesOutput];
        fssnOctaves2 = new FastSimplexStyleNoise[nOctavesOutput];
        fssnOctavesBlend = new FastSimplexStyleNoise[nOctavesBlend];
        for (int i = 0; i < Math.max(nOctavesOutput, nOctavesBlend); i++) {
            if (i < nOctavesOutput) {
                fssnOctaves1[i] = new FastSimplexStyleNoise(randomSeed);
                fssnOctaves2[i] = new FastSimplexStyleNoise(randomSeed); //TODO I think if nOctavesBlend=0, we don't even need any of octaves2
            }
            if (i < nOctavesBlend) fssnOctavesBlend[i] = new FastSimplexStyleNoise(randomSeed);
        }
        ...
        this.biomeSplerpBase = SplerpBase.forStride(horizontalBiomeGranularityPowerOfTwo);
        ...
        // FastSimplexStyleNoise provides a 2D function with a slightly more lattice-symmetric gradient set,
        ...
        disklessDepositor = new DisklessDepositor(net.minecraftforge.registries.ForgeRegistries.BIOMES.getValues(), randomSeed);
    }
    ...
    public void makeBase(IWorld worldIn, IChunk chunkIn) {
        ...
        for (int zc = 0; zc < horizontalBiomeSourceSections; zc++) {
            for (int xc = 0; xc < horizontalBiomeSourceSections; xc++) {
                ...
                SplerpBase.BiSplerp biSplerp1 = new SplerpBase.BiSplerp(biomeSplerpBase, biomeSplerpBase, biSplerpValues1);
                SplerpBase.BiSplerp biSplerp2 = new SplerpBase.BiSplerp(biomeSplerpBase, biomeSplerpBase, biSplerpValues2);

                for (int z = 0; z < horizontalBiomeGranularity; z++) {
                    SplerpBase.Splerp splerp1 = biSplerp1.at(z);
                    SplerpBase.Splerp splerp2 = biSplerp2.at(z);
    ...
    private double computeGoodEnoughNoise(int worldX, int worldY, int worldZ, double startingValue) {
        ...
        if (nOctavesBlend != 0) {
            ...
            do {
                blendingValue += fssnOctavesBlend[octave].noise3_PlaneFirst(worldX * hScaleBlend * freq, worldZ * hScaleBlend * freq, worldY * vScaleBlend * freq) * amp;
                freq *= 2.0;
                amp /= 2.0;
                octave++;
        ...
        // Compute regular noise(s). We don't always need to calculate all of the octaves. And most of the time, we only need to calculate one of the two noises.
        {
            ...
            do {
                if (blendingValue < 1) actualValue += (1 - blendingValue) * fssnOctaves1[octave].noise3_PlaneFirst(worldX * hScale * freq, worldZ * hScale * freq, worldY * vScale * freq) * amp;
                if (blendingValue > 0) actualValue += blendingValue * fssnOctaves2[octave].noise3_PlaneFirst(worldX * hScale * freq, worldZ * hScale * freq, worldY * vScale * freq) * amp;
                freq *= 2.0;
                amp /= 2.0;
                octave++;
